...
- 2025-10-30: Resume fix — corrected questionnaire/primer theming with `toRgb`, persisted the saved question set, hardened resume targeting, and restored the boot flow so navigation renders beneath the splash while auth persistence decides between Dashboard and Login.
...
- 2025-12-10: Added avatar support with Supabase Storage bucket `user-upload` (placeholder image `placeHolderUserImage.png`). UI now lets users pick or capture a profile photo in Settings and saves `avatar_url` on `public.profiles`. **Schema update required:** `ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS avatar_url text;` Ensure Storage RLS allows authenticated users to upload to `user-upload/avatars/*`. Explore/Test screens render avatars with the bucket placeholder fallback.
- 2025-12-11: Storage RLS updated for `user-upload` (auth select/insert/update/delete). Avatar uploads now work in Settings, and Explore/Test render uploaded avatars. Default recommendation filters start disabled for broader pools.
- 2025-12-11: Added lightweight caching helpers (`services/cache.ts`) and a caching strategy doc. Explore/Test screens cache recommendation pools for 7 days with cache-first load, then refresh. Media blobs stay in Supabase Storage; avatar URLs treated as long-lived. Logout surfaced in Settings.
- 2025-12-15: Introduced ELO groundwork: schema migration adds `elo_rating` and `match_allow_elo` to `profiles`, plus `match_events` table with RLS. New edge function `match-update` updates ELO on like/skip and logs events. `recommend-users` now blends PCA similarity with ELO proximity (80/20) when allowed; client passes `useElo` and lets users toggle ELO in Settings. Explore/Test requests now include the toggle and send match updates on swipe.
